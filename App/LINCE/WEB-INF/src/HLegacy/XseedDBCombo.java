...
{   
    ...
    /* --> Arrays que guardam valores items e valores do Dbcombo */
   ...
   throws Exception
{
        String wBuffer = ">";
        out.println(wBuffer);
        Refresh();        //Ler tabela e carrega o combo com o submitColumn e listColumn
   ...
   throws Exception
{
   ...
   */
   if ( tabindex != -9999) { wBuffer = wBuffer + " tabindex=\"" + tabindex + "\""; }
   /*
...
public void openPanel ()   throws Exception
{
   // Panel 
   ...
   wBuffer = wBuffer + " top:" + xsd.cStr(xsd.twipsperPixel(top)) + "px;" +
                       " left:" + xsd.cStr(xsd.twipsperPixel(left)) + "px;" + 
                       " height:20px;" +
                       " width:" + xsd.cStr(xsd.twipsperPixel(width)) + "px;" +
   ...
   throws Exception
{
    ...
    wBuffer = "<input type=\"text\" name=\"" + name + "\"" +
              " id=\"" + name + "\""  +              
              " maxlength=\"" + xsd.cStr(length) + "\"" +
              " style = \"position:absolute; border:0; "+
              " top:0px;" +
              " left:0px;" + 
              " height:18px;" +
              " width:" + xsd.cStr(xsd.twipsperPixel(width)-20) + "px;" +
              //" font-family:Verdana;font-size:11;font-style:normal;font-weight:normal;\"" + 
              " visibility:hidden;" +
              "\">";
    ...
    for (int j=0; j < eventmaxindex;j++)
    {   if (eventname[j] != null)
        {   if ( ((eventname[j].trim()).toUpperCase()).equals("ONCLICK") == false)
            {   wBuffer = wBuffer + eventname[j] + "=\" ";
                wBuffer = wBuffer + eventmethod[j] + "\" ";
            }    
        }   
    ...
    {   if (event[j] != null)
         {  if ( ((event[j].trim()).toUpperCase()).equals("ONCLICK") == false) 
            wBuffer = wBuffer + event[j] + "=\"";
             wBuffer = wBuffer + method[j] + "\"";
         }   
    }              
          
    wBuffer = wBuffer + " onKeyUp=\"" + name + "_txtKey(this.value)\">" ;
...
throws Exception
{
    ...
    wBuffer = "<img name=\"" + name + "_img\"" +
              ...
              " height:16px;" +
              " top: 1px;" +
              " left:" + xsd.cStr(xsd.twipsperPixel(width)-19) + "px;" +
              " width:16px;" ;
    if (visible  == false)   
    {   wBuffer = wBuffer + "visibility:hidden;";
    }    
   ...
   throws Exception
{
    ...
    int height=0;
    Refresh();
    if (listColumnArray.size() < 30) 
    {   size =  listColumnArray.size();
    }
    else
    {   size = 30;                 
    }
    ...
    wBuffer = "<select  name=\"" + name + "_lst\"" +
               " id =\"" + name + "_lst\"" + 
               " size=\"" + xsd.cStr(size) + "\"" +
               " style = \"position:absolute;" +                 
               " left:" +  xsd.cStr(xsd.twipsperPixel(left)) + "px;" +
               " top:" + xsd.cStr(xsd.twipsperPixel(top) + 22) + "px;" +
               " width:" + xsd.cStr(xsd.twipsperPixel(width)) + "px;" ;
    if (height>0)
    {
        wBuffer = wBuffer + " height:" +  height + "px;" ;
    }               
    wBuffer = wBuffer  + " visibility:Hidden;\"" +
               " onblur=\"" + name + "_listBlur()\"" +
               " onclick=\"" + name + "_listClick() " ;
    ...
    for (int j=0; j < eventmaxindex;j++)
    {   if (eventname[j] != null)
        {   if ( ((eventname[j].trim()).toUpperCase()).equals("ONCLICK") == true)
            {   wBuffer = wBuffer + ";" ;
                wBuffer = wBuffer + eventmethod[j] ;
            }    
        }   
    ...
    for (int j=0; j < event.length;j++)
    {   if (event[j] != null)
         {  if ( ((event[j].trim()).toUpperCase()).equals("ONCLICK") == true) 
            wBuffer = wBuffer + ";" ;
            wBuffer = wBuffer + method[j] ;
         }   
    }              
    ...
    out.println(wBuffer);               
     ...
     for ( i=0; i < listColumnArray.size(); i++)
     {   
         if (listColumnArray.get(i) == null)
         {
           break;
